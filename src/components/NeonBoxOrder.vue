<template>
  <div class="neon-root">
    <section class="neon-section">
      <div class="neon-bg-img" aria-hidden="true"></div>
      <div class="neon-inner">
        ...
        <div class="page-head">
          ...
          <h1 class="page-title">Pesan <em>Neon Box</em></h1>
          <p class="page-desc">Neon Box membantu bisnis Anda tampil lebih menonjol terutama pada malam hari melalui media promosi visual yang menarik dan efektif.</p>
        </div>

        <div class="product-banner">
          <img src="/produk/neonbox.jpeg" alt="Neon Box" class="banner-img" />
          <div class="banner-overlay">
            <span class="banner-tag">Mulai Rp 1.800.000 / unit</span>
            <p>Backlight · Akrilik · Berbagai ukuran · Custom</p>
          </div>
        </div>
        ...
        <div v-if="!customerFilled" class="form-card">
          ...
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <textarea v-model="customer.address" placeholder="Masukkan alamat lengkap untuk pengiriman / survey lokasi pemasangan" class="input-field textarea" rows="4"></textarea>
              <p class="msg-hint">
                Alamat akan digunakan untuk survey lokasi pemasangan neon box.<br>
                <span>Harga belum termasuk biaya pengiriman / pemasangan kecuali dipilih.</span>
              </p>
            </div>
          </div>
          <div class="card-foot">
        ...
        <div v-for="(order, index) in orders" :key="index" class="form-card">
          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <div class="material-grid">
                ...
                >
                  ...
                  <p v-if="material.price > 0" class="material-card__price">{{ formatCurrency(material.price) }} / unit</p>
                  <p v-else class="material-card__custom">Hubungi untuk harga</p>
                </div>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <p class="msg-hint">Pilih jenis produk terlebih dahulu. Minimal 1 piece</p>
            </div>

            <div class="form-group">
              <label class="label-main">Tambahan (Opsional)</label>

              <div class="addition-section">
                <h4 class="addition-title">Ukuran Tiang</h4>
                <p class="msg-hint addition-hint">Klik pilihan yang sama untuk membatalkan</p>
                <div class="addition-grid">
                  <div
                    v-for="(pole, key) in poleOptions"
                    :key="key"
                    class="addition-card"
                    :class="{ 'addition-card--selected': order.pole === key }"
                    @click="selectPole(index, key)"
                  >
                    <div class="addition-card__check" v-if="order.pole === key">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                        <polyline points="20 6 9 17 4 12"/>
                      </svg>
                    </div>
                    <p class="addition-card__name">{{ pole.name }}</p>
                    <p class="addition-card__price">{{ formatCurrency(pole.price) }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="addition-section">
                ...
                <div v-if="!order.pole" class="addition-locked">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2"/>
                    <path d="M7 11V7a5 5 0 0 1 10 0v4"/>
                  </svg>
                  <span>Pilih ukuran tiang terlebih dahulu</span>
                </div>
                <div v-else class="addition-synced">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                  <span>Jasa pemasangan disesuaikan otomatis dengan tiang yang dipilih</span>
                </div>
                <div class="addition-grid" :class="{ 'addition-grid--locked': !order.pole }">
                  <div
                    v-for="(installation, key) in installationOptions"
                    :key="key"
                    class="addition-card"
                    :class="{
                      'addition-card--selected': order.installation === key,
                      'addition-card--locked': !order.pole || !isSamePoleSize(order.pole, key)
                    }"
                  >
                    <div class="addition-card__check" v-if="order.installation === key">
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                        <polyline points="20 6 9 17 4 12"/>
                      </svg>
                    </div>
                    <p class="addition-card__name">{{ installation.name }}</p>
                    <p class="addition-card__price">{{ formatCurrency(installation.price) }}</p>
                  </div>
                </div>
                <p v-if="order.pole" class="msg-hint addition-hint">Klik pilihan yang sama untuk membatalkan</p>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                placeholder="Contoh: teks yang ingin ditampilkan, warna, desain khusus, dll. (opsional)"
            ...
            <div v-if="order.material" class="price-summary">
              ...
              <div class="price-row">
                ...
                <span>{{ order.material === 'custom' ? 'Hubungi Kami' : formatCurrency(order.productPrice) }}</span>
              </div>
              <div v-if="order.pole" class="price-row">
                <span>Tiang ({{ poleOptions[order.pole].name }})</span>
                <span>{{ formatCurrency(poleOptions[order.pole].price) }}</span>
              </div>
              <div v-if="order.installation" class="price-row">
                <span>Pemasangan ({{ installationOptions[order.installation].name }})</span>
                <span>{{ formatCurrency(installationOptions[order.installation].price) }}</span>
              </div>
              <div class="price-divider"></div>
              <div class="price-row price-total">
                <span>Total Pesanan #{{ index + 1 }}</span>
                <span>{{ order.material === 'custom' ? 'Hubungi Kami' : formatCurrency(order.totalPrice) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="customerFilled && orders.length > 0" class="grand-card">
          <div class="grand-head">
            ...
            <div>
              ...
              <p class="grand-amount">{{ hasCustomOrder ? 'Hubungi Kami' : formatCurrency(grandTotal) }}</p>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      materialOptions: {
        neon60x90print:  { name: 'Backlight Printing 60x90 cm',         price: 1800000 },
        neon60x90back:   { name: 'Backlight Cutting Stiker 60x90 cm',   price: 2350000 },
        neon60x90akr:    { name: 'Akrilik Cutting Stiker 60x90 cm',     price: 2850000 },
        neon125x80print: { name: 'Backlight Printing 125x80 cm',        price: 2300000 },
        neon125x80back:  { name: 'Backlight Cutting Stiker 125x80 cm',  price: 2800000 },
        neon125x80akr:   { name: 'Akrilik Cutting Stiker 125x80 cm',    price: 3300000 },
        neonD60akr:      { name: 'Akrilik Cutting Stiker Ø 60 cm',      price: 2500000 },
        neonD80akr:      { name: 'Akrilik Cutting Stiker Ø 80 cm',      price: 3000000 },
        neonD100akr:     { name: 'Akrilik Cutting Stiker Ø 100 cm',     price: 3500000 },
        custom:          { name: 'Neon Box Ukuran Custom',               price: 0 }
      },
      poleOptions: {
        pole25: { name: 'Tiang 2.5 inch', price: 350000 },
        pole30: { name: 'Tiang 3 inch',   price: 500000 },
        pole40: { name: 'Tiang 4 inch',   price: 700000 }
      },
      installationOptions: {
        install25: { name: 'Tiang 2.5 inch', price: 350000 },
        install30: { name: 'Tiang 3 inch',   price: 400000 },
        install40: { name: 'Tiang 4 inch',   price: 500000 }
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    selectPole(index, key) {
      if (this.orders[index].pole === key) {
        this.orders[index].pole = null; this.orders[index].installation = null
      } else {
    ...
    addOrder() {
      this.orders.push({ material: '', quantity: 1, pole: null, installation: null, komentar: '', productPrice: 0, totalPrice: 0 })
    },
    ...
    calculatePrice(index) {
      ...
      if (!order.material || !order.quantity) { order.productPrice = 0; order.totalPrice = 0; return }
      ...
      if (order.pole) total += this.poleOptions[order.pole].price
      if (order.installation) total += this.installationOptions[order.installation].price
    ...
    sendToWhatsApp() {
      ...
      this.orders.forEach((order, i) => {
        msg += `\n*Pesanan #${i + 1}*\n`
        msg += `Jenis Produk: ${this.materialOptions[order.material].name}\n`
        msg += `Kuantitas: ${order.quantity} piece\n`
        if (order.material === 'custom') {
          msg += `Harga Produk: Mohon konfirmasi untuk harga custom\n`
        } else {
          msg += `Harga Produk: ${this.formatCurrency(order.productPrice)}\n`
        }
        if (order.pole) msg += `Tambahan Tiang: ${this.poleOptions[order.pole].name} (${this.formatCurrency(this.poleOptions[order.pole].price)})\n`
        if (order.installation) msg += `Jasa Pemasangan: ${this.installationOptions[order.installation].name} (${this.formatCurrency(this.installationOptions[order.installation].price)})\n`
        if (order.komentar && order.komentar.trim()) msg += `Komentar: ${order.komentar.trim()}\n`
        msg += order.material === 'custom' ? `Total: Mohon konfirmasi\n` : `Total: ${this.formatCurrency(order.totalPrice)}\n`
      })
      msg += `\n━━━━━━━━━━━━━━━━━━━━\n`
      msg += this.hasCustomOrder
        ? `*TOTAL KESELURUHAN: Mohon konfirmasi untuk harga custom*\n\n`
        : `*TOTAL KESELURUHAN: ${this.formatCurrency(this.grandTotal)}*\n\n`
...
.neon-root { font-family: 'Montserrat', sans-serif; background: #f4f4f0; min-height: 100vh; }

.neon-section { position: relative; padding: 100px 20px 140px; min-height: 100vh; }
...
.neon-inner { position: relative; z-index: 1; max-width: 820px; margin: 0 auto; }
...
.addition-hint { margin-top: 0; margin-bottom: 10px; font-style: italic; }
...
.material-card__name { font-size: 0.82rem; font-weight: 700; color: #111827; margin: 0 0 3px; line-height: 1.4; transition: color 0.2s ease; }
.material-card--selected .material-card__name { color: #fff; }

.material-card__price { font-size: 0.82rem; font-weight: 800; color: #1d4ed8; margin: 0; transition: color 0.2s ease; }
.material-card--selected .material-card__price { color: #93c5fd; }

.material-card__custom { font-size: 0.78rem; font-weight: 700; color: #f59e0b; margin: 0; font-style: italic; transition: color 0.2s ease; }
.material-card--selected .material-card__custom { color: #fde68a; }

.addition-section { margin-top: 20px; }
.addition-section:first-of-type { margin-top: 12px; }
.addition-title { font-size: 0.8rem; font-weight: 700; color: #374151; margin-bottom: 6px; }
...
.addition-synced {
  ...
  color: #166534; font-size: 0.775rem; font-weight: 600;
}

.addition-card {
  position: relative; background: #f9fafb; border: 1.5px solid #e5e7eb;
  border-radius: 11px; padding: 12px 14px; cursor: pointer; transition: all 0.2s ease;
}

.addition-card:hover:not(.addition-card--locked) {
  border-color: #111827; background: #fff;
  transform: translateY(-2px); box-shadow: 0 5px 12px rgba(0,0,0,0.06);
}

.addition-card--selected {
  background: #111827; border-color: #111827;
  box-shadow: 0 6px 16px rgba(17,24,39,0.18); transform: translateY(-2px);
}

.addition-card--locked { opacity: 0.45; cursor: not-allowed; }

.addition-card__check {
  position: absolute; top: 8px; right: 8px;
  width: 18px; height: 18px; background: #1d4ed8; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  color: #fff; animation: popIn 0.2s ease both;
}

.addition-card__name { font-size: 0.8rem; font-weight: 700; color: #111827; margin: 0 0 3px; line-height: 1.4; transition: color 0.2s ease; }
.addition-card--selected .addition-card__name { color: #fff; }

.addition-card__price { font-size: 0.8rem; font-weight: 800; color: #059669; margin: 0; transition: color 0.2s ease; }
.addition-card--selected .addition-card__price { color: #6ee7b7; }
...
.guide-img-badge {
  ...
  padding: 3px 9px; border-radius: 999px; z-index: 1;
}
...
.sub-list li {
  font-size: 0.75rem; color: #78350f; line-height: 1.5;
  padding: 3px 0;
}
...
@media (max-width: 768px) {
  .neon-section { padding: 88px 16px 120px; }
