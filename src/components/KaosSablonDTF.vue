<template>
  <div class="dtf-root">
    <section class="dtf-section">
      <div class="dtf-bg-img" aria-hidden="true"></div>
      <div class="dtf-inner">
        ...
        <div class="page-head">
          ...
          <h1 class="page-title">Pesan <em>Kaos Sablon DTF</em></h1>
          <p class="page-desc">Kaos sablon DTF menghasilkan cetakan warna tajam, detail tinggi, dan tahan lama di berbagai jenis kain. Cocok untuk seragam, merchandise, dan keperluan promosi.</p>
        </div>

        <div class="product-banner">
          <img src="/produk/kaosdtf.png" alt="Kaos Sablon DTF" class="banner-img" />
          <div class="banner-overlay">
            <span class="banner-tag">Mulai Rp 100.000 / pcs</span>
            <p>DTF · Full color · A4 · F4 · A3 · Tahan lama</p>
          </div>
        </div>
        ...
        <div v-for="(order, index) in orders" :key="index" class="form-card">
          ...
          <div class="card-body">
            <div class="form-group">
              <label class="label-main">Pilih Ukuran Sablon <span class="req">*</span></label>
              <div class="material-grid">
                ...
                >
                  ...
                  <div class="material-card__icon">
                    <div class="paper-icon" :class="`paper-icon--${key}`">
                      <span class="paper-label">{{ material.label }}</span>
                    </div>
                  </div>
                  <p class="material-card__name">{{ material.name }}</p>
                  <p class="material-card__dim">{{ material.dimension }}</p>
                  <p class="material-card__price">{{ formatCurrency(material.price) }} / pcs</p>
                </div>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <p class="msg-hint">Jumlah kaos yang dipesan</p>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                placeholder="Contoh: posisi sablon, warna kaos, ukuran kaos (S/M/L/XL), dll. (opsional)"
            ...
            <div v-if="order.material" class="price-summary">
              <div class="price-row">
                ...
                <span>{{ materialOptions[order.material].name }} ({{ materialOptions[order.material].dimension }})</span>
              </div>
              <div class="price-row">
                <span>Harga per pcs</span>
                <span>{{ formatCurrency(materialOptions[order.material].price) }}</span>
              </div>
              <div class="price-row">
                <span>Kuantitas</span>
...
export default {
  ...
  data() {
    return {
      ...
      errorMessage: '',
      materialOptions: {
        a4: { name: 'Ukuran A4', label: 'A4', dimension: '21 × 29.7 cm', price: 100000 },
        f4: { name: 'Ukuran F4', label: 'F4', dimension: '21.5 × 33 cm', price: 110000 },
        a3: { name: 'Ukuran A3', label: 'A3', dimension: '29.7 × 42 cm', price: 150000 }
      }
    }
  },
  computed: {
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `DTF-${ts.slice(-4)}${rand}`
    },
    saveCustomerData() {
    ...
    addOrder() {
      this.orders.push({ material: '', quantity: 1, komentar: '', totalPrice: 0 })
    },
    ...
    calculatePrice(index) {
      ...
      o.totalPrice = this.materialOptions[o.material].price * o.quantity
    },
    formatCurrency(n) {
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!o.material) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon pilih ukuran sablon pada pesanan #${i + 1}!`); return
        }
        if (!o.quantity) {
          this.showError('Kuantitas Belum Diisi', `Mohon isi kuantitas pada pesanan #${i + 1}!`); return
        }
      }
      let msg = `*PESANAN KAOS SABLON DTF*\n`
      ...
      this.orders.forEach((o, i) => {
        msg += `\n*Pesanan #${i + 1}*\n`
        msg += `Ukuran Sablon: ${this.materialOptions[o.material].name} (${this.materialOptions[o.material].dimension})\n`
        msg += `Harga per pcs: ${this.formatCurrency(this.materialOptions[o.material].price)}\n`
...
.dtf-root { font-family: 'Montserrat', sans-serif; background: #f4f4f0; min-height: 100vh; }
.dtf-section { position: relative; padding: 100px 20px 140px; min-height: 100vh; }
.dtf-bg-img { position: fixed; inset: 0; background-image: url('@/assets/balibg.png'); background-size: cover; background-position: center; opacity: 0.05; pointer-events: none; z-index: 0; }
.dtf-inner { position: relative; z-index: 1; max-width: 820px; margin: 0 auto; }
...
.material-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 4px; }
.material-card { position: relative; background: #f9fafb; border: 1.5px solid #e5e7eb; border-radius: 14px; padding: 18px 14px 16px; cursor: pointer; transition: all 0.25s ease; text-align: center; }
.material-card:hover { border-color: #111827; background: #fff; transform: translateY(-2px); box-shadow: 0 6px 18px rgba(0,0,0,0.08); }
.material-card--selected { background: #111827; border-color: #111827; box-shadow: 0 8px 22px rgba(17,24,39,0.2); transform: translateY(-2px); }
...
.material-card__icon { display: flex; justify-content: center; margin-bottom: 12px; }
.paper-icon { display: flex; align-items: center; justify-content: center; border-radius: 4px; border: 2px solid #d1d5db; background: #fff; font-family: 'Montserrat', sans-serif; font-weight: 900; color: #374151; transition: all 0.2s ease; }
.paper-icon--a4 { width: 34px; height: 48px; font-size: 0.65rem; }
.paper-icon--f4 { width: 36px; height: 50px; font-size: 0.62rem; }
.paper-icon--a3 { width: 48px; height: 34px; font-size: 0.65rem; }
.paper-label { line-height: 1; }
.material-card--selected .paper-icon { border-color: rgba(255,255,255,0.3); background: rgba(255,255,255,0.08); color: #fff; }

.material-card__name { font-size: 0.88rem; font-weight: 800; color: #111827; margin: 0 0 3px; transition: color 0.2s ease; }
.material-card--selected .material-card__name { color: #fff; }
.material-card__dim { font-size: 0.7rem; font-weight: 600; color: #9ca3af; margin: 0 0 5px; transition: color 0.2s ease; }
.material-card--selected .material-card__dim { color: rgba(255,255,255,0.5); }
.material-card__price { font-size: 0.8rem; font-weight: 800; color: #1d4ed8; margin: 0; transition: color 0.2s ease; }
...
.price-row span:last-child { font-weight: 700; color: #fff; text-align: right; max-width: 60%; }
...
@media (max-width: 768px) {
  .dtf-section { padding: 88px 18px 120px; }
  .dtf-inner { max-width: 100%; }
  .product-banner { height: 175px; margin-bottom: 20px; border-radius: 16px; }
  ...
  .material-grid { grid-template-columns: repeat(3, 1fr); gap: 10px; }
  .material-card { padding: 16px 10px 14px; }
  .material-card__name { font-size: 0.82rem; }
  .material-card__dim { font-size: 0.65rem; }
  .material-card__price { font-size: 0.75rem; }
...
@media (max-width: 480px) {
  .dtf-section { padding: 80px 14px 120px; }
  .product-banner { height: 148px; border-radius: 14px; }
  .back-btn { padding: 7px 13px; font-size: 0.75rem; margin-bottom: 16px; }
  .page-title { font-size: clamp(1.5rem, 6.5vw, 2rem); }
  ...
  .material-grid { grid-template-columns: 1fr; gap: 9px; }
  .material-card { display: flex; flex-direction: row; align-items: center; gap: 16px; text-align: left; padding: 14px 16px; border-radius: 12px; }
  .material-card__icon { margin-bottom: 0; flex-shrink: 0; }
  .material-card__check { top: 50%; transform: translateY(-50%); right: 12px; }
  .paper-icon--a4 { width: 28px; height: 40px; font-size: 0.6rem; }
  .paper-icon--f4 { width: 30px; height: 42px; font-size: 0.58rem; }
  .paper-icon--a3 { width: 40px; height: 28px; font-size: 0.6rem; }
  .material-card__name { font-size: 0.88rem; margin-bottom: 2px; }
  .material-card__dim { font-size: 0.7rem; margin-bottom: 3px; }
