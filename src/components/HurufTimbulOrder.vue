<template>
  <div class="huruf-root">
    <section class="huruf-section">
      <div class="huruf-bg-img" aria-hidden="true"></div>
      <div class="huruf-inner">
        ...
        <div class="page-head">
          ...
          <h1 class="page-title">Pesan <em>Huruf Timbul</em></h1>
          <p class="page-desc">Media branding visual tiga dimensi yang diproduksi secara presisi, menggunakan material berkualitas untuk hasil yang kuat, rapi, dan tahan lama.</p>
        </div>

        <div class="product-banner">
          <img src="/produk/huruftimbul.png" alt="Huruf Timbul" class="banner-img" />
          <div class="banner-overlay">
            <p>Custom desain · 3 dimensi · Material berkualitas · Tahan lama</p>
          </div>
        </div>
        ...
        <div v-if="!customerFilled" class="form-card">
          ...
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <textarea v-model="customer.address" placeholder="Masukkan alamat lengkap untuk survey lokasi pemasangan" class="input-field textarea" rows="4"></textarea>
              <p class="msg-hint">
                Alamat akan digunakan untuk survey lokasi pemasangan huruf timbul.
              </p>
            </div>
          </div>
          <div class="card-foot">
        ...
        <div v-for="(order, index) in orders" :key="index" class="form-card">
          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <div class="material-grid">
                <div
                  ...
                  :key="key"
                  class="material-card"
                  :class="{ 'material-card--selected': order.product === key }"
                  @click="selectProduct(index, key)"
                >
                  <div class="material-card__check" v-if="order.product === key">
                  ...
                  <p class="material-card__name">{{ product.name }}</p>
                  <p class="material-card__note">Hubungi untuk harga</p>
                </div>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <div class="qty-wrap">
                <button type="button" class="qty-btn" @click="decreaseQty(index)" :disabled="order.quantity <= 1 || !order.product">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <line x1="5" y1="12" x2="19" y2="12"/>
                  </svg>
                </button>
                <input v-model.number="order.quantity" type="number" min="1" class="qty-input" :disabled="!order.product" />
                <button type="button" class="qty-btn" @click="increaseQty(index)" :disabled="!order.product">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                placeholder="Contoh: teks huruf, ukuran, warna, material, lokasi pemasangan, dll. (opsional)"
            ...
            <div v-if="order.product" class="screenshot-guide">
              ...
              <div class="guide-body">
                <p class="guide-warning">
                  Screenshot bagian <strong>ringkasan pesanan</strong> di bawah ini sebagai bukti lampiran pemesanan.
                ...
                <div class="guide-img-wrap">
                  ...
                  <p class="guide-img-caption">Screenshot bagian ringkasan pesanan seperti contoh di atas</p>
                </div>
                <div class="guide-steps">
                  <p class="steps-title">Langkah-langkah:</p>
                  <ol>
                    <li><span class="snum">1</span><span>Selesaikan pemilihan detail pesanan</span></li>
                    <li><span class="snum">2</span><span>Geser ke bawah hingga menemukan ringkasan pesanan</span></li>
            ...
            <div v-if="order.product" class="price-summary">
              <div class="price-row">
                <span>Jenis Produk</span>
                <span>{{ productNames[order.product] }}</span>
              </div>
              <div class="price-row">
                <span>Kuantitas</span>
        ...
        <div v-if="customerFilled && orders.length > 0" class="grand-card">
          <div class="grand-head">
            ...
            <div>
              ...
              <p class="grand-amount">Hubungi Kami</p>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      errorMessage: '',
      productOptions: {
        huruftimbul: { name: 'Huruf Timbul Custom' }
      },
      productNames: {
  ...
  methods: {
    ...
    addOrder() {
      this.orders.push({ product: '', quantity: 1, komentar: '' })
    },
    removeOrder(index) { this.orders.splice(index, 1) },
    increaseQty(index) { this.orders[index].quantity++ },
    decreaseQty(index) {
      if (this.orders[index].quantity > 1) this.orders[index].quantity--
    },
    confirmReset() {
    ...
    sendToWhatsApp() {
      ...
      this.orders.forEach((order, i) => {
        msg += `\n*Pesanan #${i + 1}*\n`
        msg += `Jenis Produk: ${this.productNames[order.product]}\n`
        msg += `Kuantitas: ${order.quantity} unit\n`
        if (order.komentar && order.komentar.trim()) msg += `Komentar: ${order.komentar.trim()}\n`
      })
      msg += `\n━━━━━━━━━━━━━━━━━━━━\n`
      msg += `\nSegera konfirmasi saya terkait ketersediaan dan harga yang harus saya bayar. Terima kasih!`
...
.huruf-root { font-family: 'Montserrat', sans-serif; background: #f4f4f0; min-height: 100vh; }

.huruf-section { position: relative; padding: 100px 20px 140px; min-height: 100vh; }
...
.huruf-inner { position: relative; z-index: 1; max-width: 820px; margin: 0 auto; }
...
.page-desc { font-size: 0.95rem; font-weight: 500; color: #6b7280; line-height: 1.7; max-width: 500px; }
...
.material-card {
  ...
  border-radius: 13px; padding: 16px; cursor: pointer; transition: all 0.25s ease;
}
...
.material-card__name { font-size: 0.88rem; font-weight: 700; color: #111827; margin: 0 0 4px; transition: color 0.2s ease; }
.material-card--selected .material-card__name { color: #fff; }

.material-card__note { font-size: 0.78rem; font-weight: 600; color: #f59e0b; margin: 0; font-style: italic; transition: color 0.2s ease; }
.material-card--selected .material-card__note { color: #fde68a; }
...
.sub-list li {
  font-size: 0.75rem; color: #78350f; line-height: 1.5; padding: 3px 0;
}
...
.price-total span:last-child { font-weight: 900 !important; font-size: 0.9rem; color: #fde68a !important; }
...
.grand-amount { font-size: 1.3rem; font-weight: 900; color: #fff; margin: 0; }
...
@media (max-width: 768px) {
  .huruf-section { padding: 88px 16px 120px; }
  .product-banner { height: 165px; }
  .grand-amount { font-size: 1.1rem; }
...
@media (max-width: 480px) {
  .huruf-section { padding: 78px 14px 120px; }
