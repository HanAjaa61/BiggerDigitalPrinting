<template>
  <div class="cs-root">
    <section class="cs-section">
      <div class="cs-bg-img" aria-hidden="true"></div>
      <div class="cs-inner">
        ...
        <div class="page-head">
          ...
          <h1 class="page-title">Pesan <em>Cutting Stiker</em></h1>
          <p class="page-desc">Cutting stiker presisi tinggi untuk kebutuhan dekorasi, branding, dan promosi. Tersedia dalam 1, 2, hingga 3 warna dengan hasil rapi dan tahan lama. Harga dihitung berdasarkan luas area (panjang × lebar).</p>
        </div>

        <div class="product-banner">
          <img src="/produk/cuttingstiker.png" alt="Cutting Stiker" class="banner-img" />
          <div class="banner-overlay">
            <span class="banner-tag">Mulai Rp 75 / cm²</span>
            <p>1 warna · 2 warna · 3 warna · Custom ukuran</p>
          </div>
        </div>
        ...
        <div v-for="(order, index) in orders" :key="index" class="form-card">
          ...
          <div class="card-body">
            <div class="form-group">
              <label class="label-main">Pilih Jumlah Warna <span class="req">*</span></label>
                ...
                >
                  ...
                  <div class="material-card__icon">
                    <div class="color-dots">
                      <span
                        v-for="n in material.colorCount"
                        :key="n"
                        class="color-dot"
                        :style="{ background: material.dotColors[n - 1] }"
                      ></span>
                    </div>
                  </div>
                  <p class="material-card__name">{{ material.name }}</p>
                  <p class="material-card__price">Rp {{ material.pricePerCm.toLocaleString('id-ID') }} / cm²</p>
                </div>
              </div>
            </div>

            <div class="form-group">
              <label class="label-main">Ukuran <span class="req">*</span></label>
              <div class="size-wrap">
                <div class="size-input-group">
                  <label class="size-label">Panjang</label>
                  <div class="size-input-row">
                    <input
                      v-model.number="order.panjang"
                      type="number"
                      min="1"
                      placeholder="0"
                ...
                <div class="size-times">×</div>
                <div class="size-input-group">
                  <label class="size-label">Lebar</label>
                  <div class="size-input-row">
                    <input
                      v-model.number="order.lebar"
                      type="number"
                      min="1"
                      placeholder="0"
                      class="input-field size-input"
                ...
                <div class="size-result">
                  <label class="size-label">Luas</label>
                  <div class="size-result-value">
                    <span class="size-result-num">{{ order.panjang && order.lebar ? (order.panjang * order.lebar).toLocaleString('id-ID') : '—' }}</span>
                    <span class="size-result-unit">cm²</span>
                  </div>
                </div>
              </div>
              <p v-if="!order.material" class="msg-hint">Pilih jumlah warna terlebih dahulu</p>
              <p v-else class="msg-hint">Harga = luas × Rp {{ materialOptions[order.material].pricePerCm.toLocaleString('id-ID') }} / cm²</p>
            </div>

            <div class="form-group">
              <label>Kuantitas <span class="req">*</span></label>
              <div class="qty-wrap">
                <button type="button" class="qty-btn" @click="decreaseQty(index)" :disabled="order.quantity <= 1 || !order.material">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <line x1="5" y1="12" x2="19" y2="12"/>
                  </svg>
                </button>
                <input v-model.number="order.quantity" type="number" min="1" class="qty-input" @input="calculatePrice(index)" :disabled="!order.material" />
              ...
              <p class="msg-hint">Jumlah lembar / pcs yang dipesan</p>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                placeholder="Contoh: warna spesifik, bentuk potongan, bahan, penggunaan, dll. (opsional)"
            ...
            <div v-if="order.material && order.panjang && order.lebar" class="screenshot-guide">
            ...
            <div v-if="order.material && order.panjang && order.lebar" class="price-summary">
              <div class="price-row">
              ...
              <div class="price-row">
                <span>Ukuran</span>
                <span>{{ order.panjang }} × {{ order.lebar }} cm</span>
              </div>
              <div class="price-row">
                <span>Luas</span>
                <span>{{ (order.panjang * order.lebar).toLocaleString('id-ID') }} cm²</span>
              </div>
              <div class="price-row">
                <span>Harga per cm²</span>
                <span>Rp {{ materialOptions[order.material].pricePerCm.toLocaleString('id-ID') }}</span>
              </div>
              <div class="price-row">
                <span>Harga per lembar</span>
                <span>{{ formatCurrency(order.pricePerSheet) }}</span>
              </div>
              <div class="price-row">
                <span>Kuantitas</span>
...
export default {
    ...
    return {
      ...
      materialOptions: {
        ...
        warna2: {
          ...
          pricePerCm: 95,
          colorCount: 2,
          dotColors: ['#111827', '#1d4ed8']
        },
        warna3: {
          name: '3 Warna',
          pricePerCm: 150,
          colorCount: 3,
          dotColors: ['#111827', '#1d4ed8', '#ef4444']
        }
      }
    }
  },
  ...
  methods: {
    generateOrderCode() {
      ...
      this.orderCode = `CST-${ts.slice(-4)}${rand}`
    },
    saveCustomerData() {
    ...
    addOrder() {
      this.orders.push({ material: '', panjang: null, lebar: null, quantity: 1, komentar: '', pricePerSheet: 0, totalPrice: 0 })
    },
    ...
    calculatePrice(index) {
      ...
      if (!o.material || !o.panjang || !o.lebar || !o.quantity) { o.pricePerSheet = 0; o.totalPrice = 0; return }
      const luas = o.panjang * o.lebar
      o.pricePerSheet = luas * this.materialOptions[o.material].pricePerCm
      o.totalPrice = o.pricePerSheet * o.quantity
    },
    formatCurrency(n) {
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (!o.material) {
          this.showError('Data Pesanan Tidak Lengkap', `Mohon pilih jumlah warna pada pesanan #${i + 1}!`); return
        }
        if (!o.panjang || !o.lebar) {
          this.showError('Ukuran Belum Diisi', `Mohon isi panjang dan lebar pada pesanan #${i + 1}!`); return
        }
        if (!o.quantity) {
          this.showError('Kuantitas Belum Diisi', `Mohon isi kuantitas pada pesanan #${i + 1}!`); return
        }
      }
      let msg = `*PESANAN CUTTING STIKER*\n`
      ...
      this.orders.forEach((o, i) => {
        const luas = o.panjang * o.lebar
        msg += `\n*Pesanan #${i + 1}*\n`
        msg += `Jumlah Warna: ${this.materialOptions[o.material].name}\n`
        msg += `Ukuran: ${o.panjang} × ${o.lebar} cm (${luas.toLocaleString('id-ID')} cm²)\n`
        msg += `Harga per cm²: Rp ${this.materialOptions[o.material].pricePerCm.toLocaleString('id-ID')}\n`
        msg += `Harga per lembar: ${this.formatCurrency(o.pricePerSheet)}\n`
...
.cs-root { font-family: 'Montserrat', sans-serif; background: #f4f4f0; min-height: 100vh; }
.cs-section { position: relative; padding: 100px 20px 140px; min-height: 100vh; }
.cs-bg-img { position: fixed; inset: 0; background-image: url('@/assets/balibg.png'); background-size: cover; background-position: center; opacity: 0.05; pointer-events: none; z-index: 0; }
.cs-inner { position: relative; z-index: 1; max-width: 820px; margin: 0 auto; }
...
.material-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-top: 4px; }
.material-card { position: relative; background: #f9fafb; border: 1.5px solid #e5e7eb; border-radius: 13px; padding: 16px 14px; cursor: pointer; transition: all 0.25s ease; text-align: center; }
...
.material-card__icon { display: flex; justify-content: center; margin-bottom: 10px; }
.color-dots { display: flex; gap: 5px; justify-content: center; align-items: center; }
.color-dot { width: 16px; height: 16px; border-radius: 50%; display: inline-block; border: 2px solid rgba(255,255,255,0.5); }
.material-card--selected .color-dot { border-color: rgba(255,255,255,0.3); }
.material-card__name { font-size: 0.85rem; font-weight: 800; color: #111827; margin: 0 0 4px; transition: color 0.2s ease; }
.material-card--selected .material-card__name { color: #fff; }
.material-card__price { font-size: 0.78rem; font-weight: 700; color: #1d4ed8; margin: 0; transition: color 0.2s ease; }
.material-card--selected .material-card__price { color: #93c5fd; }

.size-wrap { display: flex; align-items: flex-end; gap: 12px; flex-wrap: wrap; }
.size-input-group { display: flex; flex-direction: column; gap: 6px; flex: 1; min-width: 100px; }
.size-label { font-size: 0.72rem; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: 0.5px; }
.size-input-row { display: flex; align-items: center; gap: 6px; }
.size-input { flex: 1; padding: 11px 12px; }
.size-unit { font-size: 0.8rem; font-weight: 700; color: #9ca3af; white-space: nowrap; }
.size-times { font-size: 1.4rem; font-weight: 900; color: #d1d5db; padding-bottom: 4px; align-self: flex-end; flex-shrink: 0; padding-bottom: 12px; }
.size-result { display: flex; flex-direction: column; gap: 6px; flex: 1; min-width: 100px; }
.size-result-value { display: flex; align-items: center; gap: 6px; background: #f0f9ff; border: 1.5px solid #bae6fd; border-radius: 11px; padding: 11px 13px; }
.size-result-num { font-size: 1rem; font-weight: 800; color: #0369a1; }
.size-result-unit { font-size: 0.78rem; font-weight: 700; color: #0369a1; opacity: 0.7; }

.qty-wrap { display: flex; align-items: stretch; border: 1.5px solid #e5e7eb; border-radius: 11px; overflow: hidden; height: 46px; background: #f9fafb; }
.qty-btn { background: #fff; border: none; width: 42px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #6b7280; transition: all 0.2s ease; }
.qty-btn:hover:not(:disabled) { background: #111827; color: #fff; }
.qty-btn:disabled { opacity: 0.3; cursor: not-allowed; }
.qty-input { flex: 1; border: none; text-align: center; font-family: 'Montserrat', sans-serif; font-size: 0.95rem; font-weight: 700; color: #111827; background: transparent; min-width: 0; }
...
@media (max-width: 768px) {
  .cs-section { padding: 88px 18px 120px; }
  .cs-inner { max-width: 100%; }
  .product-banner { height: 175px; margin-bottom: 20px; }
  .page-head { margin-bottom: 24px; }
  .page-desc { font-size: 0.88rem; max-width: 100%; }
  .form-card { margin-bottom: 14px; border-radius: 16px; }
  .card-head { padding: 15px 18px; gap: 10px; }
  .card-body { padding: 18px; }
  .card-foot { padding: 0 18px 18px; }
  .form-group { margin-bottom: 18px; }
  .material-grid { grid-template-columns: repeat(3, 1fr); gap: 8px; }
  .material-card { padding: 14px 10px; }
  .material-card__name { font-size: 0.78rem; }
  .material-card__price { font-size: 0.72rem; }
  .color-dot { width: 13px; height: 13px; }
  .size-wrap { gap: 10px; }
  .size-input-group { min-width: 80px; }
  .qty-wrap { height: 48px; }
  .qty-btn { width: 46px; }
  .grand-card { border-radius: 16px; margin-bottom: 16px; }
  .grand-head { padding: 16px 18px; gap: 12px; }
  .grand-amount { font-size: 1.35rem; }
  .grand-icon { width: 42px; height: 42px; }
  .grand-actions { grid-template-columns: 1fr; gap: 9px; padding: 16px 18px; }
  .btn-outline, .btn-danger, .btn-wa { padding: 13px 16px; font-size: 0.85rem; justify-content: center; }
  .price-summary { padding: 16px; margin-top: 16px; }
  .screenshot-guide { margin-top: 18px; }
  .guide-body { padding: 15px; }
  .guide-steps { padding: 12px; }
  .dialog-box { padding: 28px 22px; border-radius: 18px; }
  .dialog-actions { gap: 10px; }
}

@media (max-width: 480px) {
  .cs-section { padding: 80px 14px 120px; }
  .product-banner { height: 150px; border-radius: 16px; }
  .back-btn { padding: 7px 13px; font-size: 0.75rem; margin-bottom: 16px; }
  .page-title { font-size: clamp(1.6rem, 7vw, 2.2rem); }
  .page-desc { font-size: 0.85rem; line-height: 1.65; }
  .form-card { margin-bottom: 12px; border-radius: 14px; }
  .card-head { padding: 14px 16px; gap: 10px; }
  .card-head-icon { width: 30px; height: 30px; border-radius: 8px; }
  .card-title { font-size: 0.88rem; }
  .card-body { padding: 16px; }
  .card-foot { padding: 0 16px 16px; }
  .form-group { margin-bottom: 16px; }
  .form-group label, .label-main { font-size: 0.77rem; margin-bottom: 8px; }
  .input-field { padding: 11px 13px; font-size: 0.845rem; border-radius: 10px; }
  ...
  .material-card__name { font-size: 0.85rem; margin-bottom: 2px; }
  .material-card__price { font-size: 0.78rem; }
  .color-dots { gap: 4px; }
  .color-dot { width: 14px; height: 14px; }
  .size-wrap { flex-direction: column; gap: 8px; align-items: stretch; }
  .size-input-group { min-width: unset; width: 100%; }
  .size-input-row { gap: 8px; }
  .size-input { width: 100%; }
  .size-times { align-self: center; padding: 0; transform: rotate(90deg); font-size: 1.2rem; color: #d1d5db; }
  .size-result { width: 100%; }
  .size-result-value { justify-content: center; padding: 10px; }
  .size-result-num { font-size: 1.1rem; }
  .qty-wrap { height: 50px; }
  .qty-btn { width: 50px; }
  .qty-input { font-size: 1rem; }
  .screenshot-guide { margin-top: 16px; border-radius: 12px; }
  .guide-head { padding: 11px 14px; gap: 8px; }
  .guide-head h4 { font-size: 0.78rem; }
  .guide-body { padding: 14px; }
  .guide-warning { font-size: 0.77rem; padding: 10px 12px; }
  .guide-img-box img { max-width: 180px; }
  .guide-steps { padding: 12px; margin-bottom: 10px; }
  .guide-steps ol li { padding: 8px 10px; font-size: 0.77rem; gap: 8px; }
  .snum { width: 20px; height: 20px; min-width: 20px; font-size: 0.68rem; }
  .guide-footer { padding: 9px 12px; font-size: 0.72rem; }
  .price-summary { padding: 14px; border-radius: 11px; margin-top: 14px; }
  .price-row { font-size: 0.79rem; padding: 3px 0; }
  .price-row span:last-child { font-size: 0.82rem; }
  .price-total span:last-child { font-size: 0.95rem !important; }
  .grand-card { border-radius: 14px; }
  .grand-head { padding: 14px 16px; gap: 11px; }
  .grand-icon { width: 38px; height: 38px; border-radius: 10px; }
  .grand-label { font-size: 0.72rem; }
  .grand-amount { font-size: 1.2rem; }
  .grand-actions { padding: 14px 16px; gap: 8px; }
  .btn-outline, .btn-danger, .btn-wa { padding: 12px 14px; font-size: 0.82rem; border-radius: 10px; gap: 6px; }
  .dialog-backdrop { padding: 16px; }
  .dialog-box { padding: 24px 18px; border-radius: 16px; max-width: 100%; }
  .dialog-icon { width: 48px; height: 48px; margin-bottom: 12px; }
  .dialog-title { font-size: 1.05rem; }
  .dialog-msg { font-size: 0.82rem; margin-bottom: 18px; }
  .dialog-actions { flex-direction: column; gap: 8px; }
  .dialog-actions .btn-outline,
  .dialog-actions .btn-danger { width: 100%; padding: 13px; }
  .btn-remove { padding: 5px 9px; font-size: 0.72rem; }
}
</style>