<template>
  <div class="foto-root">
    <section class="foto-section">
      <div class="foto-bg-img" aria-hidden="true"></div>
      <div class="foto-inner">
        ...
        <div class="page-head">
          ...
          <h1 class="page-title">Pesan <em>Foto</em></h1>
          <p class="page-desc">Cetak foto dengan bahan luster berkualitas tinggi, tekstur halus dan efek semi-gloss yang memberikan kesan mewah dan elegan.</p>
        </div>

        <div class="product-banner">
          <img src="/produk/luster.png" alt="Luster" class="banner-img" />
          <div class="banner-overlay">
            <span class="banner-tag">Mulai Rp 150.000 / lembar</span>
            <p>Kualitas tinggi · Tekstur halus · Semi-gloss elegan</p>
          </div>
        </div>
        ...
        <div v-for="(order, index) in orders" :key="index" class="form-card">
          ...
          <div class="card-body">
            <div class="form-group">
              ...
              <div class="material-grid">
                ...
                >
                  ...
                  <p class="material-card__price">{{ formatCurrency(product.price) }} / m²</p>
                </div>
              </div>
            </div>
            ...
            <div class="form-row-2">
              <div class="form-group">
                <label>Panjang (meter) <span class="req">*</span></label>
                <div class="input-unit-wrap">
                  <input
                    ...
                    placeholder="1.5"
                    class="input-field"
                    :class="{ 'input-error': order.lengthError }"
                    @input="validateAndCalculate(index)"
                  />
                  <span class="unit-badge">m</span>
                </div>
                <p v-if="order.lengthError" class="msg-error">{{ order.lengthError }}</p>
                <p v-else class="msg-hint">Minimal 1 meter</p>
              </div>

              <div class="form-group">
                <label>Lebar (meter) <span class="req">*</span></label>
                <div class="input-unit-wrap">
                  <input
                    v-model="order.width"
                    type="number"
                    ...
                    placeholder="1.0"
                    class="input-field"
                    :class="{ 'input-error': order.widthError }"
                    @input="validateAndCalculate(index)"
                  />
                  <span class="unit-badge">m</span>
                </div>
                <p v-if="order.widthError" class="msg-error">{{ order.widthError }}</p>
                <p v-else class="msg-hint">Min 0.5m (50cm) - Maks 1.5m (150cm)</p>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                placeholder="Contoh: pastikan warna kulit natural, cetak portrait, resolusi tinggi, dll. (opsional)"
            ...
            <div v-if="order.product" class="price-summary">
              <div class="price-row">
                <span>Luas Area</span>
                <span>{{ order.area ? order.area.toFixed(2) : '0' }} m²</span>
              </div>
              <div class="price-row">
...
export default {
  ...
  data() {
    return {
      ...
      errorMessage: '',
      productOptions: {
        luster: { name: 'Luster', price: 150000 }
      },
      productNames: {
        luster: 'Luster'
      }
  ...
  methods: {
    ...
    validateAndCalculate(index) {
      ...
      if (order.length && !isNaN(length)) {
        if (length < 1) { order.lengthError = 'Panjang minimal 1 meter'; hasError = true }
        else { order.lengthError = '' }
      } else { order.lengthError = '' }
      const width = parseFloat(order.width)
      if (order.width && !isNaN(width)) {
        if (width < 0.5) { order.widthError = 'Lebar minimal 0.5 meter (50 cm)'; hasError = true }
        else if (width > 1.5) { order.widthError = 'Lebar maksimal 1.5 meter (150 cm)'; hasError = true }
        else { order.widthError = '' }
      } else { order.widthError = '' }
      if (!hasError) { this.calculatePrice(index) }
      else { order.area = 0; order.unitPrice = 0; order.totalPrice = 0 }
    },
    calculatePrice(index) {
      ...
      if (!order.product || !order.length || !order.width || !order.quantity) {
        order.area = 0; order.unitPrice = 0; order.totalPrice = 0; return
      }
      let length = parseFloat(order.length)
      let width = parseFloat(order.width)
      if (isNaN(length) || isNaN(width)) { order.area = 0; order.unitPrice = 0; order.totalPrice = 0; return }
    ...
    sendToWhatsApp() {
      ...
      for (let i = 0; i < this.orders.length; i++) {
        ...
        if (order.lengthError || order.widthError) {
          this.showError('Data Pesanan Tidak Valid', `Mohon perbaiki ukuran pada pesanan #${i + 1}!`); return
        }
      }
      let msg = `*PESANAN FOTO / LUSTER*\n`
      ...
      this.orders.forEach((order, i) => {
        ...
        msg += `Ukuran: Panjang ${order.length}m x Lebar ${order.width}m (${order.area.toFixed(2)} m²)\n`
        msg += `Kuantitas: ${order.quantity} lembar\n`
...
.foto-root { font-family: 'Montserrat', sans-serif; background: #f4f4f0; min-height: 100vh; }

.foto-section { position: relative; padding: 100px 20px 140px; min-height: 100vh; }
...
.foto-inner { position: relative; z-index: 1; max-width: 820px; margin: 0 auto; }
...
.qty-wrap {
  ...
  border: 1.5px solid #e5e7eb; border-radius: 11px;
  overflow: hidden; height: 46px; background: #f9fafb;
}
...
@media (max-width: 768px) {
  .foto-section { padding: 88px 16px 120px; }
...
@media (max-width: 480px) {
  .foto-section { padding: 78px 14px 120px; }
